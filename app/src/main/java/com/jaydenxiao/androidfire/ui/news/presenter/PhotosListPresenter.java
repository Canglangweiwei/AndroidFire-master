...
public class PhotosListPresenter extends PhotoListContract.Presenter {
    ...
    public void getPhotosListDataRequest(int size, int page) {
        mRxManage.add(mBasePresenterModel.getPhotosListData(size, page).subscribe(new RxSubscriber<List<PhotoGirl>>(mContext, false) {
            @Override
            public void onStart() {
                super.onStart();
            ...
            protected void _onNext(List<PhotoGirl> photoGirls) {
                mBasePresenterRootView.returnPhotosListData(photoGirls);
